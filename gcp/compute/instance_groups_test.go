...
var _ = Describe("InstanceGroups", func() {
	...
	Describe("Delete", func() {
		BeforeEach(func() {
			...
			client.ListInstanceGroupsCall.Returns.Output = &gcpcompute.InstanceList{
		...
		It("deletes instance groups", func() {
			...
			Expect(client.DeleteInstanceGroupCall.Receives.Instance).To(Equal("banana"))

			Expect(logger.PrintfCall.Messages).To(Equal([]string{"SUCCESS deleting instance group banana\n"}))
		})

		Context("when the instance group has tags", func() {
			BeforeEach(func() {
				client.ListInstanceGroupsCall.Returns.Output = &gcpcompute.InstanceList{
					Items: []*gcpcompute.Instance{{
						Name: "banana",
			...
			It("uses them in the prompt", func() {
				...
				Expect(logger.PromptCall.Receives.Message).To(Equal("Are you sure you want to delete instance group banana (banana-director)?"))
			})
		})
